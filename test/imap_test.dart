...
void main() {
  ...
  final intIMaps = intMaps.map((m) => IMap.from(IntOrder, m));
  ...
  test("create from pairs", () => qc.check(forall(intIMaps, (dynamicM) {
    ...
    final mFromPairs = IMap.fromPairs(mPairs, IntOrder);
    return m == mFromPairs;
  })));
  ...
  test("key lookup", () {
    ...
    final complexIMaps = intIMaps.map((m) => m.foldLeftKV<IMap<Tuple2<int, int>, int>>(IMap.empty(o), (acc, k, v) => acc.put(tuple2(k, -k), v)));
  ...
  test("create from iterables", () => qc.check(forall(intIMaps, (dynamicM) {
    final m = dynamicM;
    return m == IMap.fromIterables(m.keyIterable(), m.valueIterable(), IntOrder);
  })));
  ...
  test("cata", () => qc.check(forall(intIMaps, (dynamicM) {
    final m = dynamicM;
    final cataed = m.cata<IMap<int, int>>(IMap.empty(IntOrder), id, (acc, k, v, cataLeft, cataRight) => cataRight(cataLeft(acc.put(k, v))));
    return m == cataed;
  })));

  group("IMapTr", () => checkTraversableLaws(IMapTr, intIMaps));

  group("imapMonoid(IListMi)", () => checkMonoidLaws(imapMonoidWithOrder<int, IList<int>>(ilistMi<int>(), IntOrder), c.ints.map((i) => IMap.from(IntOrder, {i: ilist([i])}))));

  group("IMapMi", () => checkMonoidLaws(IMapMi, c.ints.map((i) => IMap.from(comparableOrder(), {i: i}))));
