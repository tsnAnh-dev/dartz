...
void main() {
  ...
  final intILists = intLists.map((l) => IList.from(l));

  test("grab bag", () {
    final l = iota(3).map((i) => i + 1);

    final l2 = l.flatMap<int>((i) => Cons(i * 2, Cons(i * 2, nil())));
    expect(IListMi.append(l2, l2.reverse()),
        ilist([2, 2, 4, 4, 6, 6, 6, 6, 4, 4, 2, 2]));
    expect(l2.foldLeft<int>(0, (a, b) => a + b), 24);
/*
    final Monad<Option<IList>> OptionIListM = IListTMonad(OptionMP as Monad<Option<IList>>);
    final ol = some(l);
    final stackedResult = OptionIListM.bind(ol, (i) => i % 2 == 1 ? some(Cons("$i!", nil<String>())) : some(nil()));
    expect(stackedResult, some(ilist(["1!", "3!"])));
*/
    final IList<int> nums =
        ilist([743, 59, 633, 532, 744, 234, 792, 891, 178, 356]);
    Tuple4<int, int, int, int> mapper(int i) => Tuple4(i, 1, i, i);
    final reducer = tuple4Semigroup(NumSumMi, NumSumMi, NumMinSi, NumMaxSi);
    Tuple3<num, num, num> polish(num sum, num count, num min, num max) =>
        Tuple3(sum / count, min, max);
    final result = nums.foldMapO(reducer, mapper).map(tuplize4(polish));
    expect(result, some(Tuple3(516.2, 59, 891)));

    IList<int> dup(int i) => Cons<int>(i, Cons<int>(i, nil()));
    expect(l.flatMap(dup), ilist([1, 1, 2, 2, 3, 3]));
    expect(l.flatMap(dup), l.foldMap(IListMi, dup));

    expect(IList.sequenceEither(IList.from([right(1), right(2), right(3)])),
        right(ilist([1, 2, 3])));

    final l1 = ilist([1, 2, 3, 4]);
    expect(l1.traverseEither((i) => i < 4 ? right(i) : left("too big")),
        left("too big"));
    expect(IList.sequenceEither(l1.map((i) => right(i))),
        right(ilist([1, 2, 3, 4])));
    ...
    expect(
        IList.flattenOption(
            ilist([1, 2, 3, 4]).map((i) => i % 2 == 1 ? some(i) : none())),
        ilist([1, 3]));
  ...
  test('length', () {
    qc.check(forall(intLists, (l) => (l).length == ilist(l).length()));
  });
  ...
  test('reverse', () {
    qc.check(forall2(
        intILists,
        intILists,
        (IList<int> xs, IList<int> ys) =>
            xs.plus(ys).reverse() == ys.reverse().plus(xs.reverse())));
  });
  ...
  test('bind', () {
    qc.check(forall(
        ...
        (IList<int> l) =>
            l.bind((i) => ilist([i, i])) ==
            IList.flattenIList(l.map((i) => ilist([i, i])))));
  });

  test('traverse', () {
    expect(ilist([2, 4, 6, 8]).traverseOption((i) => option(i % 2 == 0, i)),
        some(ilist([2, 4, 6, 8])));
    expect(ilist([2, 3, 4, 6]).traverseOption((i) => option(i % 2 == 0, i)),
        none());
  });

  test('sequence', () {
    expect(
        IList.sequenceOption(
            ilist([2, 4, 6, 8]).map((i) => option(i % 2 == 0, i))),
        some(ilist([2, 4, 6, 8])));
    expect(
        IList.sequenceOption(
            ilist([2, 3, 4, 6]).map((i) => option(i % 2 == 0, i))),
        none());
  });
  ...
  test("stack safety (traverse and bind)", () async {
    final EM = EvaluationMonad(UnitMi);
    final IList<int> massive =
        IdM.replicate(10000, 1).flatMap((i) => ilist([i, i]));
    expect(
        await massive
            .traverseEvaluation(UnitMi, (i) => EM.modify((s) => s + i))
            .state(unit, 0),
        right(20000));
  });
  ...
  test("to/from iterable", () {
    qc.check(
        forall(intILists, (l) => l == IList.from((l as IList).toIterable())));
  });
  ...
  test("to option", () {
    qc.check(forall(intILists, (dynamicL) {
      final l = dynamicL;
      final empty = l.option.fold(ifNone: () => true, ifSome: (_) => false);
      return empty == (l == nil());
    }));
  });

  test("asCons, head and tail", () {
    int sumThroughAsCons(IList<int> l) => l.asCons().fold(
        ifNone: () => 0, ifSome: (c) => c.head + sumThroughAsCons(c.tail));
  ...
  test(
      "isEmpty",
      () => qc.check(forall(
          intILists, (IList<int> il) => (il.length() == 0) == il.isEmpty)));
}
