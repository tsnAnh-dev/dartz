...
abstract mixin class Foldable<F> {
  ...
  B foldRight<A, B>(F fa, B z, B f(A a, B previous)) =>
      foldMap<A, Endo<B>>(endoMi(), fa, curry2(f))(z);

  B foldRightWithIndex<A, B>(F fa, B z, B f(int i, a, B previous)) => foldRight(
      fa,
      tuple2(z, length(fa) - 1),
      (a, Tuple2<B, int> t) =>
          tuple2(f(t.value2, a, t.value1), t.value2 - 1)).value1;

  B foldLeft<A, B>(F fa, B z, B f(B previous, A a)) =>
      foldMap<A, Endo<B>>(dualEndoMi(), fa, curry2(flip(f)))(z);

  B foldLeftWithIndex<A, B>(F fa, B z, B f(B previous, int i, A a)) => foldLeft(
      fa,
      tuple2(z, 0),
      (Tuple2<B, int> t, A a) =>
          tuple2(f(t.value1, t.value2, a), t.value2 + 1)).value1;

  Option<B> foldMapO<A, B>(Semigroup<B> si, F fa, B f(A a)) =>
      foldMap(OptionMonoid(si), fa, composeF(some, f));
  ...
  A intercalate<A>(Monoid<A> mi, F fa, A a) =>
      foldRight<A, Option<A>>(
          fa,
          none(),
          (A ca, oa) => some(
