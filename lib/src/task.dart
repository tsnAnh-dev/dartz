...
class Task<A> implements MonadCatchOps<Task, A> {
  ...
  @override Task<B> bind<B>(Function1<A, Task<B>> f) => Task(() => _run().then((a) => f(a).run()));

  Task<B> pure<B>(B b) => Task(() => Future.value(b));

  @override Task<Either<Object, A>> attempt() => Task(() => run().then((a) => right<Object, A>(a)).catchError((err) => left<Object, A>(cast(err))));

  @override Task<A> fail(Object err) => Task(() => Future.error(err));

  @override Task<B> map<B>(B f(A a)) => Task(() => _run().then(f));
  ...
  @override Task<B> flatMap<B>(Function1<A, Task<B>> f) => Task(() => _run().then((a) => f(a).run()));
...
class TaskMonadCatch extends Functor<Task> with Applicative<Task>, Monad<Task>, MonadCatch<Task> {
  ...
  @override Task<A> fail<A>(Object err) => Task(() => Future.error(err));

  @override Task<A> pure<A>(A a) => Task(() => Future.microtask(() => a));
}
