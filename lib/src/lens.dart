...
class Lens<AIn, AOut, BIn, BOut> {
  ...
  Lens<AIn, AOut, CIn, COut> andThen<CIn, COut>(Lens<BOut, BIn, CIn, COut> otherLens) => Lens((a) => otherLens.get(get(a)), (a, c) => set(a, otherLens.set(get(a), c)));
}

Lens<AIn, AOut, BIn, BOut> lens<AIn, AOut, BIn, BOut>(Function1<AIn, BOut> getter, Function2<AIn, BIn, AOut> setter) => Lens(getter, setter);

class SimpleLens<A, B> extends Lens<A, A, B, B> {
  SimpleLens(Function1<A, B> getter, Function2<A, B, A> setter): super(getter, setter);
  SimpleLens<A, C> andThenS<C>(Lens<B, B, C, C> otherLens) => SimpleLens((a) => otherLens.get(get(a)), (a, c) => set(a, otherLens.set(get(a), c)));
  OptionLens<A, C> andThenO<C>(OptionLens<B, C> otherLens) => OptionLens((a) => otherLens.get(get(a)), (a, c) => otherLens.set(get(a), c).map((b) => set(a, b)));
  EitherLens<A, C, E> andThenE<C, E>(EitherLens<B, C, E> otherLens) => EitherLens((a) => otherLens.get(get(a)), (a, c) => otherLens.set(get(a), c).map((b) => set(a, b)));
}

SimpleLens<A, B> lensS<A, B>(Function1<A, B> getter, Function2<A, B, A> setter) => SimpleLens(getter, setter);
...
class OptionLens<A, B> extends Lens<A, Option<A>, B, Option<B>> {
  ...
  OptionLens<A, C> oAndThen<C>(Lens<B, B, C, C> otherLens) => OptionLens((a) => get(a).map(otherLens.get), (a, c) => get(a).bind((b) => set(a, otherLens.set(b, c))));
  OptionLens<A, C> oAndThenO<C>(OptionLens<B, C> otherLens) => OptionLens((a) => get(a).bind(otherLens.get), (a, c) => get(a).bind((b1) => otherLens.set(b1, c)).bind((b) => set(a, b)));
  Option<A> modifyO(A a, Function1<B, B> f) => get(a).bind((b) => set(a, f(b)));
  Function1<A, Option<A>> modifyOC(Function1<B, B> f) => (a) => modifyO(a, f);
}

OptionLens<A, B> lensO<A, B>(Function1<A, Option<B>> getter, Function2<A, B, Option<A>> setter) => OptionLens(getter, setter);
...
class EitherLens<A, B, E> extends Lens<A, Either<E, A>, B, Either<E, B>> {
  ...
  EitherLens<A, C, E> eAndThen<C>(Lens<B, B, C, C> otherLens) => EitherLens((a) => get(a).map(otherLens.get), (a, c) => get(a).bind((b) => set(a, otherLens.set(b, c))));
  EitherLens<A, C, E> eAndThenE<C>(EitherLens<B, C, E> otherLens) => EitherLens((a) => get(a).bind(otherLens.get), (a, c) => get(a).bind((b1) => otherLens.set(b1, c)).bind((b) => set(a, b)));
...
EitherLens<A, B, E> lensE<A, B, E>(Function1<A, Either<E, B>> getter, Function2<A, B, Either<E, A>> setter) => EitherLens(getter, setter);
